...
export const UsersBrief = () => {
  ...
  const AssignMutation = useMutation({
    mutationFn: async (payload: {
      ...
      Client.PATCH('/api/backend/v1/customers/{customer_id}/assign', {
        params: {
  ...
  const { data, isLoading } = useGetUsersBriefQuery(options, debouncedValue)
  ...
  useEffect(() => {
    if (activeChat?.assigned.is_assigned) {
      setSelectedOption({
        name: activeChat?.assigned.assigned_to_username,
        profile_picture: '',
        user_id: activeChat?.assigned.assigned_to,
      })
    }
  }, [activeChat])
  return (
