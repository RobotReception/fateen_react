...
export const useGetTeamInboxQuery = () => {
  return useQuery({
    queryFn: () => {
      return Client.GET('/api/backend/v1/teams', {})
    },

    queryKey: [queryKeyEnum.GET_TEAM_INBOX],
    select: ({ data }) => {
      return {
        teams: data?.data.teams,
...
export const useGetCustomersQuery = (filter: object) => {
  return useQuery({
    queryFn: () => {
