...
.cf-table thead th{padding:9px 14px;font-size:10.5px;font-weight:700;color:var(--t-text-faint);text-align:right;border-bottom:1.5px solid var(--t-border);white-space:nowrap;background:#fafbfc;text-transform:uppercase;letter-spacing:.03em}
.cf-table tbody td{padding:10px 14px;font-size:12px;color:var(--t-text);border-bottom:1px solid #f0f1f3;vertical-align:middle}
.cf-table tbody tr{transition:background .1s}
.cf-table tbody tr:hover{background:rgba(27,80,145,.015)}
...
.cf-field{width:100%;padding:8px 11px;border-radius:8px;border:1.5px solid var(--t-border);background:#fafbfc;font-size:12px;color:var(--t-text);outline:none;transition:border-color .15s,box-shadow .15s;box-sizing:border-box;font-family:inherit}
.cf-field:focus{border-color:var(--t-accent);box-shadow:0 0 0 3px rgba(27,80,145,.06);background:#fff}
.cf-field::placeholder{color:#b0b7c3}
.cf-field:disabled{opacity:.55;cursor:not-allowed}

.cf-label{font-size:10px;font-weight:700;color:var(--t-text-muted);display:flex;align-items:center;gap:3px;margin-bottom:4px}
.cf-label-hint{font-size:9px;font-weight:500;color:#b0b7c3;margin-right:auto}

.cf-btn-primary{display:inline-flex;align-items:center;gap:5px;padding:7px 14px;border-radius:8px;border:none;background:var(--t-brand-orange);color:#fff;font-size:12px;font-weight:700;cursor:pointer;transition:all .12s;font-family:inherit;box-shadow:0 2px 8px rgba(27,80,145,.15)}
.cf-btn-primary:hover:not(:disabled){opacity:.9;box-shadow:0 4px 12px rgba(27,80,145,.2)}
.cf-btn-primary:disabled{opacity:.5;cursor:not-allowed}

.cf-btn-ghost{display:inline-flex;align-items:center;gap:5px;padding:6px 12px;border-radius:8px;border:1.5px solid var(--t-border);background:#fff;color:var(--t-text);font-size:11px;font-weight:600;cursor:pointer;transition:all .12s;font-family:inherit}
...
.cf-actions-btn{width:28px;height:28px;border-radius:7px;border:1px solid var(--t-border);background:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--t-text-faint);transition:all .1s}
.cf-actions-btn:hover{background:var(--t-surface);color:var(--t-text-muted);border-color:var(--t-border-medium)}

.cf-actions-menu{position:absolute;left:0;top:100%;margin-top:2px;z-index:20;background:#fff;border:1px solid var(--t-border);border-radius:10px;box-shadow:0 8px 30px rgba(0,0,0,.1);min-width:130px;padding:4px;animation:cfMenuIn .1s ease-out}
.cf-actions-menu button{width:100%;padding:7px 10px;border:none;background:transparent;cursor:pointer;display:flex;align-items:center;gap:6px;border-radius:7px;font-size:11px;font-weight:600;color:var(--t-text);transition:background .08s;font-family:inherit;text-align:right}
...
.cf-type-badge{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:6px;font-size:10px;font-weight:700;border:1px solid var(--t-border)}
.cf-active-dot{width:6px;height:6px;border-radius:50%;display:inline-block}

.cf-select{width:100%;padding:8px 11px;border-radius:8px;border:1.5px solid var(--t-border);background:#fafbfc;font-size:12px;color:var(--t-text);outline:none;font-family:inherit;cursor:pointer;appearance:auto;transition:border-color .15s,box-shadow .15s}
.cf-select:focus{border-color:var(--t-accent);box-shadow:0 0 0 3px rgba(27,80,145,.06)}
...
.cf-toggle-track.off{background:var(--t-border-medium)}
...
const TYPE_COLORS: Record<DynamicFieldType, string> = {
    text: "var(--t-text-muted)", number: "var(--t-accent-secondary)", email: "var(--t-warning)", phone: "#16a34a",
    date: "#8b5cf6", boolean: "#ec4899", select: "var(--t-accent)", multi_select: "#0891b2",
    url: "#ea580c", textarea: "var(--t-text-muted)",
}
    ...
    return (
        <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 90, background: "rgba(0,0,0,.35)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div onClick={e => e.stopPropagation()} dir="rtl" style={{
                borderRadius: 16, background: "#fff", border: "1px solid var(--t-border)",
                ...
                <div style={{
                    ...
                    <button onClick={onClose} style={{
                        width: 26, height: 26, borderRadius: 7, background: "var(--t-surface)",
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 20, alignItems: "center", padding: "6px 0" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 600, color: "var(--t-text-muted)" }}>حقل مطلوب</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <button type="button" className={`cf-toggle-track ${isActive ? "on" : "off"}`} onClick={() => setIsActive(!isActive)} />
                    <span style={{ fontSize: 11, fontWeight: 600, color: "var(--t-text-muted)" }}>نشط</span>
                </div>
            </div>
    ...
    return (
        <div style={{ direction: "rtl" }}>
            ...
            <div style={{
                ...
                borderRadius: 10, background: "#fafbfc", border: "1px solid var(--t-border)",
            }}>
                ...
                <div style={{ flex: 1 }}>
                    <span style={{ fontSize: 11, color: "var(--t-text-muted)", lineHeight: 1.5 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexShrink: 0 }}>
                    <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: 16, fontWeight: 800, color: "var(--t-accent)" }}>{totalCount}</div>
                        <div style={{ fontSize: 9, color: "var(--t-text-faint)", fontWeight: 600 }}>الإجمالي</div>
                    </div>
                    <div style={{ width: 1, background: "var(--t-border)" }} />
                    <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: 16, fontWeight: 800, color: "#16a34a" }}>{activeCount}</div>
                        <div style={{ fontSize: 9, color: "var(--t-text-faint)", fontWeight: 600 }}>نشط</div>
                    </div>
                </div>
            </div>
            ...
            <div style={{ borderRadius: 12, border: "1px solid var(--t-border)", background: "#fff", overflow: "visible" }}>
                {isLoading ? (
                    <div style={{ textAlign: "center", padding: "48px 0", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, color: "var(--t-text-faint)", fontSize: 12 }}>
                        <div style={{ width: 24, height: 24, borderRadius: "50%", border: "2.5px solid var(--t-border)", borderTopColor: "var(--t-accent)", animation: "spin .7s linear infinite" }} />
                        جاري التحميل...
                    </div>
                ) : filtered.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "48px 0" }}>
                        <div style={{ width: 52, height: 52, borderRadius: 14, background: "var(--t-surface)", margin: "0 auto 10px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <FileSliders size={22} style={{ color: "var(--t-border-medium)" }} />
                        </div>
                        <div style={{ fontSize: 14, color: "var(--t-text)", fontWeight: 700, marginBottom: 3 }}>
                    ...
                    <table className="cf-table">
                        ...
                        <tbody>
                            {filtered.map((f: DynamicField) => {
                                const typeColor = TYPE_COLORS[f.field_type] || "var(--t-text-muted)"
                                return (
                                    <tr key={f.id ?? f.field_name} style={{ animation: "cfFade .2s ease-out" }}>
                                        <td><span style={{ fontSize: 11, fontWeight: 700, color: "var(--t-text-faint)" }}>{f.display_order ?? "—"}</span></td>
                                        <td>
                                            <span style={{ fontWeight: 700, fontSize: 12.5, color: "var(--t-text)" }}>{f.field_label || f.field_name}</span>
                                        </td>
                                        <td>
                                            <code style={{ fontSize: 10, color: "var(--t-text-muted)", background: "var(--t-surface)", padding: "2px 7px", borderRadius: 5, fontFamily: "monospace" }}>
                                                {f.field_name}
                                            </code>
                                        </td>
                                        <td><span style={{ color: "var(--t-text-faint)", fontSize: 11 }}>{f.description || "—"}</span></td>
                                        <td>
                                        ...
                                        <td>
                                            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                                                <span className="cf-active-dot" style={{ background: f.is_active ? "#16a34a" : "var(--t-border-medium)" }} />
                                                <span style={{ fontSize: 11, fontWeight: 600, color: f.is_active ? "#16a34a" : "var(--t-text-faint)" }}>
                                                    {f.is_active ? "نشط" : "معطل"}
                                                </span>
                                            </div>
                                        </td>
                                        <td><span style={{ fontSize: 11, color: "var(--t-text-faint)", whiteSpace: "nowrap" }}>{fmtDate(f.created_at)}</span></td>
                                        <td>
            ...
            {deleteTarget && (
                <Modal title="تأكيد الحذف" width={380} onClose={() => setDeleteTarget(null)}>
                    <div style={{ textAlign: "center", padding: "4px 0" }}>
                        ...
                        <div style={{ fontSize: 14, fontWeight: 700, color: "var(--t-text)", marginBottom: 4 }}>
