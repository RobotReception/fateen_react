    ...
    return (
        <>
                    ...
                    {openDrop === "assign" && (
                        <DropPanel anchorRef={assignRef} onClose={closeDrop} width={230}>
                            ...
                            {filteredUsers.length === 0 && (
                                <p style={{ fontSize: 11, color: "var(--t-text-faint)", padding: "8px 12px", margin: 0, textAlign: "center" }}>لا توجد نتائج</p>
                            )}
                            {filteredUsers.map(u => (
                ...
                <>
                    ...
                    {openDrop === "team" && (
                        <DropPanel anchorRef={teamRef} onClose={closeDrop} width={220}>
                            ...
                            {teams.length === 0 && (
                                <p style={{ fontSize: 11, color: "var(--t-text-faint)", padding: "8px 12px", margin: 0, textAlign: "center" }}>لا توجد فرق</p>
                            )}
                            {teams.map(t => {
                                ...
                                return (
                                    <button key={t.team_id} className="ap-drop-item" onClick={() => {
                                        ...
                                        <span style={{
                                            ...
                                            background: isActive ? "var(--t-gradient-accent)" : "#fff",
                                            display: "flex", alignItems: "center", justifyContent: "center",
                                            transition: "all .15s ease",
                                            boxShadow: isActive ? "0 1px 4px rgba(27,80,145,0.25)" : "none",
                                        }}>
                                            {isActive && (
                                        ...
                                        <span style={{ flex: 1, fontSize: 12, fontWeight: isActive ? 600 : 500, color: isActive ? "var(--t-accent)" : "var(--t-text-secondary)" }}>{t.name}</span>
                                    </button>
                                )
                            })}
                        </DropPanel>
            ...
            <style>{`
                ...
                .ap-compact-btn:hover { background:var(--t-accent-muted); border-color:var(--t-accent-muted); color:var(--t-accent); }
                .ap-compact-btn[data-active="true"] { background:var(--t-accent-muted); border-color:var(--t-accent-secondary); color:var(--t-accent); }
                .ap-compact-btn[data-accent="true"] { color:var(--t-accent); border-color:rgba(27,80,145,0.25); }
                .ap-compact-btn[data-accent="true"]:hover { background:var(--t-accent-muted); border-color:var(--t-accent-secondary); }
                ...
                .ap-icon-btn {
                    ...
                    color:var(--t-text-muted, var(--t-text-muted)); transition:all .15s;
                    position:relative;
                }
                .ap-icon-btn:hover { background:var(--t-accent-muted); border-color:var(--t-accent-muted); color:var(--t-accent); }
                .ap-icon-active { background:var(--t-accent-muted) !important; border-color:var(--t-accent-secondary) !important; color:var(--t-accent) !important; }
                ...
                .ap-reopen-btn { background:var(--t-accent-muted); color:var(--t-accent-secondary); }
                ...
                .ap-drop {
                    ...
                    box-shadow:0 12px 40px rgba(27,80,145,.08), 0 4px 12px rgba(0,0,0,.06);
                    animation:apFadeIn .12s ease-out;
                    backdrop-filter:blur(8px);
                }
                .ap-drop-title {
                    font-size:10px; font-weight:700; color:var(--t-text-muted);
                ...
                .ap-drop-item:hover { background:var(--t-accent-muted); }
                .ap-drop-item[data-active="true"] { background:var(--t-accent-muted); color:var(--t-accent); }
                .ap-drop-item[data-danger="true"] { color:var(--t-danger); }
                ...
                .ap-search-icon { color:var(--t-text-faint); flex-shrink:0; }
                .ap-search-input {
                ...
                .ap-close-panel {
                    ...
                    box-shadow:0 16px 48px rgba(27,80,145,.1), 0 4px 12px rgba(0,0,0,.06);
                    animation:apFadeIn .12s ease-out;
                }
                .ap-close-header { display:flex; align-items:center; gap:7px; margin-bottom:12px; }
                .ap-close-header svg { color:var(--t-accent); }
                .ap-close-title { font-size:13px; font-weight:700; color:var(--t-text); margin:0; }
                .ap-label { font-size:10px; font-weight:600; color:var(--t-text-secondary); display:block; margin-bottom:4px; }
                .ap-select {
                    width:100%; padding:7px 8px; border-radius:7px;
                    border:1px solid var(--t-border); background:#fafbfc;
                    color:var(--t-text); font-size:11.5px; outline:none;
                    margin-bottom:10px; cursor:pointer; font-family:inherit;
                }
                .ap-select:focus { border-color:var(--t-accent-secondary); }
                .ap-textarea {
                    width:100%; padding:7px 8px; border-radius:7px;
                    border:1px solid var(--t-border); background:#fafbfc;
                    color:var(--t-text); font-size:11.5px; outline:none; resize:vertical;
                    margin-bottom:12px; font-family:inherit; min-height:50px; line-height:1.5;
                }
                .ap-textarea:focus { border-color:var(--t-accent-secondary); }
                .ap-textarea::placeholder { color:#b0b6c0; }
                .ap-close-actions { display:flex; justify-content:flex-end; gap:5px; }
                .ap-btn-cancel {
                    padding:6px 12px; border-radius:7px; border:1px solid var(--t-border);
                    background:#fff; color:var(--t-text-muted); font-size:11px;
                    cursor:pointer; font-weight:500; font-family:inherit;
                    transition:all .12s;
                }
                .ap-btn-cancel:hover { background:var(--t-surface); }
                .ap-btn-submit {
                ...
                .ap-btn-submit:hover { background:linear-gradient(135deg, #005f9a, var(--t-accent-hover)); }
...
function DropItem({ icon, label, active, danger, onClick }: {
    ...
    return (
        <button className="ap-drop-item" onClick={onClick}
            ...
            <span style={{ display: "flex", flexShrink: 0, color: danger ? "var(--t-danger)" : active ? "var(--t-accent)" : "var(--t-text-muted)" }}>{icon}</span>
            <span style={{ flex: 1 }}>{label}</span>
            {active && <span style={{ fontSize: 10, color: "var(--t-accent)", fontWeight: 700 }}>✓</span>}
        </button>
    )
}
