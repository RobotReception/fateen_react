...
.ad-scroll::-webkit-scrollbar-thumb{background:#e5e7eb;border-radius:4px}
.ad-input{width:100%;padding:9px 12px;border-radius:10px;border:1.5px solid #e5e7eb;background:#fff;font-size:13px;color:#111827;outline:none;transition:border-color .15s,box-shadow .15s;font-family:inherit;box-sizing:border-box}
.ad-input:focus{border-color:#004786;box-shadow:0 0 0 3px rgba(0,71,134,.06)}
...
.ad-chip.sel{border-color:#004786;background:rgba(0,71,134,.06);color:#004786}
.ad-chip.unsel{border-color:#e5e7eb;background:#f9fafb;color:#9ca3af}
.ad-chip.unsel:hover{border-color:#d1d5db;color:#6b7280}
`
            ...
            <div dir="rtl" onClick={e => e.stopPropagation()} style={{ width: 440, maxHeight: "85vh", background: "#fff", borderRadius: 18, boxShadow: "0 24px 60px rgba(0,0,0,.18)", overflow: "hidden", animation: "adFade .2s ease-out" }}>
                ...
                <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #ebeef2", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 32, height: 32, borderRadius: 10, background: "linear-gradient(135deg, #004786, #0072b5)", display: "flex", alignItems: "center", justifyContent: "center" }}><Edit3 size={14} style={{ color: "#fff" }} /></div>
                        <div><div style={{ fontSize: 15, fontWeight: 800, color: "#111827" }}>تعديل الوكيل</div><div style={{ fontSize: 11, color: "#9ca3af" }}>{agent.name}</div></div>
                    </div>
                ...
                <div className="ad-scroll" style={{ padding: "18px 24px 20px", overflowY: "auto", maxHeight: "55vh" }}>
                    <div style={{ marginBottom: 14 }}><label style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", display: "block", marginBottom: 5 }}>اسم الوكيل *</label><input autoFocus className="ad-input" value={name} onChange={e => setName(e.target.value)} /></div>
                    <div style={{ marginBottom: 14 }}><label style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", display: "block", marginBottom: 5 }}>الوصف</label><input className="ad-input" value={desc} onChange={e => setDesc(e.target.value)} placeholder="وصف مختصر" /></div>
                ...
                <div style={{ padding: "14px 24px 18px", borderTop: "1px solid #ebeef2", display: "flex", gap: 8 }}>
                    <button disabled={!name.trim() || isSaving} onClick={() => onSave({ name, description: desc, status, departments: depts, categories: cats })} style={{ flex: 1, padding: "10px 0", borderRadius: 10, border: "none", background: "linear-gradient(135deg, #004786, #0072b5)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, opacity: isSaving ? .7 : 1, fontFamily: "inherit" }}>
...
export function AgentDetailPage() {
    ...
    if (isLoading) return (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", padding: 48 }}>
            <Loader2 size={28} className="animate-spin" style={{ color: "#004786" }} />
        </div>
    )
    ...
    if (!agent) return (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", padding: 48, textAlign: "center" }}>
            <AlertCircle size={36} style={{ color: "#d1d5db", marginBottom: 12 }} />
            <div style={{ fontSize: 16, fontWeight: 700, color: "#111827", marginBottom: 8 }}>الوكيل غير موجود</div>
            <button onClick={() => navigate("/dashboard/settings/organization?tab=ai")} style={{ padding: "9px 20px", borderRadius: 10, border: "none", background: "linear-gradient(135deg, #004786, #0072b5)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                العودة للوكلاء
            </button>
        </div>
    )
    ...
    return (
        <div className="flex h-full" dir="rtl">
            ...
            <aside className={`shrink-0 bg-white transition-all duration-300 ${sidebarCollapsed ? "w-[60px]" : "w-[220px]"}`}
                style={{ borderLeft: "1px solid #ebeef2", borderRadius: 6, margin: "8px 0", overflow: "hidden", display: "flex", flexDirection: "column" }}>
                        ...
                        return (
                            <button key={tab.key} onClick={() => setActiveTab(tab.key)} title={sidebarCollapsed ? tab.label : undefined}
                                style={{
                                    ...
                                    border: "none", background: isActive ? "#f3f4f6" : "transparent",
                                ...
                                <div style={{
                                    ...
                                    background: isActive ? `${c1}15` : "#f3f4f6",
                                    display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all .15s",
                                }}>
                                    <Icon size={sidebarCollapsed ? 15 : 13} strokeWidth={isActive ? 2.2 : 1.6} style={{ color: isActive ? c1 : "#9ca3af", transition: "color .15s" }} />
                                </div>
                                {!sidebarCollapsed && (
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: 12.5, fontWeight: isActive ? 600 : 500, color: isActive ? "#1f2937" : "#6b7280", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tab.label}</div>
                                        <div style={{ fontSize: 10, color: "#9ca3af" }}>{tab.desc}</div>
                                    </div>
                                )}
                            </button>
                        )
                ...
                {/* Actions */}
                {!sidebarCollapsed && (
                    <div style={{ padding: "8px 10px 12px", borderTop: "1px solid #ebeef2", display: "flex", gap: 4 }}>
                        <ActionGuard pageBit={PAGE_BITS.AGENTS} actionBit={ACTION_BITS.UPDATE_AGENT}>
                            <button onClick={() => setShowEdit(true)} style={{
                                flex: 1, padding: "7px 0", borderRadius: 8, border: "1px solid #ebeef2",
                                background: "#fff", cursor: "pointer", display: "flex", alignItems: "center",
                                justifyContent: "center", gap: 4, fontSize: 11, fontWeight: 600, color: "#6b7280",
                                transition: "all .12s", fontFamily: "inherit",
                            }}
                                onMouseEnter={e => { e.currentTarget.style.borderColor = "#004786"; e.currentTarget.style.color = "#004786" }}
                                onMouseLeave={e => { e.currentTarget.style.borderColor = "#ebeef2"; e.currentTarget.style.color = "#6b7280" }}>
                                <Edit3 size={11} /> تعديل
                            </button>
                        </ActionGuard>
                        <ActionGuard pageBit={PAGE_BITS.AGENTS} actionBit={ACTION_BITS.DELETE_AGENT}>
                            <button onClick={() => setShowDelete(true)} style={{
                                padding: "7px 12px", borderRadius: 8, border: "1px solid #ebeef2",
                                background: "#fff", cursor: "pointer", display: "flex", alignItems: "center",
                                justifyContent: "center", color: "#6b7280", transition: "all .12s",
                            }}
                ...
                <div style={{ position: "fixed", inset: 0, zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,.35)", backdropFilter: "blur(4px)" }} onClick={() => setShowDelete(false)}>
                    <div dir="rtl" onClick={e => e.stopPropagation()} style={{ width: 380, background: "#fff", borderRadius: 18, padding: 24, boxShadow: "0 24px 60px rgba(0,0,0,.18)", animation: "adFade .2s ease-out" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                            <div style={{ width: 40, height: 40, borderRadius: 12, background: "rgba(220,38,38,.06)", display: "flex", alignItems: "center", justifyContent: "center" }}><AlertCircle size={20} style={{ color: "#dc2626" }} /></div>
                            <div><div style={{ fontSize: 15, fontWeight: 800, color: "#111827" }}>حذف الوكيل</div><div style={{ fontSize: 12, color: "#6b7280" }}>"{agent.name}"</div></div>
                        </div>
                        <div style={{ fontSize: 13, color: "#6b7280", lineHeight: 1.7, marginBottom: 20 }}>هل أنت متأكد من حذف هذا الوكيل؟ سيتم حذف جميع الإعدادات نهائياً.</div>
                        <div style={{ display: "flex", gap: 8 }}>
                            <button onClick={handleDelete} style={{ flex: 1, padding: "10px 0", borderRadius: 10, border: "none", background: "#dc2626", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}><Trash2 size={13} style={{ display: "inline", verticalAlign: "-2px", marginLeft: 5 }} /> حذف نهائياً</button>
                            <button onClick={() => setShowDelete(false)} style={{ padding: "10px 18px", borderRadius: 10, border: "1.5px solid #e5e7eb", background: "#fff", color: "#6b7280", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>إلغاء</button>
                        </div>
                    </div>
                </div>,
                document.body
