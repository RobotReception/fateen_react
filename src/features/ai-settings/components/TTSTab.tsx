...
.tts2-field{width:100%;padding:8px 11px;border-radius:8px;border:1.5px solid #e5e7eb;background:#fff;font-size:12px;color:#111827;outline:none;font-family:monospace;transition:border-color .15s,box-shadow .15s;box-sizing:border-box}
.tts2-field:focus{border-color:#004786;box-shadow:0 0 0 3px rgba(0,71,134,.06)}
.tts2-lbl{font-size:10px;font-weight:700;color:#9ca3af;display:flex;align-items:center;gap:3px;margin-bottom:4px;text-transform:uppercase;letter-spacing:.06em}
.tts2-seg{flex:1;padding:7px 0;border-radius:8px;border:1.5px solid #e5e7eb;cursor:pointer;font-size:11px;font-weight:600;transition:all .12s;background:#fff;color:#6b7280;font-family:inherit}
.tts2-seg:hover{border-color:#d1d5db}
.tts2-seg.on{background:linear-gradient(135deg,#004786,#0072b5);border-color:#004786;color:#fff;box-shadow:0 1px 4px rgba(0,71,134,.12)}
`
...
function Toggle({ on, onToggle, size = "md" }: { on: boolean; onToggle: () => void; size?: "sm" | "md" }) {
    ...
    return (
        <button onClick={onToggle} style={{
            ...
            cursor: "pointer", background: on ? "linear-gradient(135deg, #004786, #0072b5)" : "#e5e7eb",
            transition: "background .2s", flexShrink: 0,
            boxShadow: on ? "0 1px 5px rgba(0,71,134,.15)" : "none",
        }}>
...
const sk = (w: string, h = 12, r = 6) => ({ width: w, height: h, borderRadius: r, background: "linear-gradient(110deg,#f0f1f3 30%,#f9fafb 50%,#f0f1f3 70%)", backgroundSize: "200% 100%", animation: "ttsShimmer 1.6s ease-in-out infinite" } as React.CSSProperties)
...
function TTSProviderCard({ name, prov, agentId }: { name: string; prov: TTSProvider; agentId: string }) {
    ...
    return (
        <div style={{
            borderRadius: 12, border: `1px solid ${open ? "rgba(0,71,134,.15)" : "#ebeef2"}`, background: "#fff",
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", cursor: "pointer" }} onClick={() => setOpen(!open)}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
                    <div style={{
                        ...
                        <Mic size={14} style={{ color: prov.enabled ? "#fff" : "#9ca3af" }} />
                    </div>
                    <div style={{ minWidth: 0, flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <span style={{ fontSize: 13, fontWeight: 700, color: "#111827" }}>{displayName}</span>
                            <span style={{ fontSize: 8.5, fontWeight: 700, padding: "2px 7px", borderRadius: 20, background: prov.enabled ? "rgba(22,163,74,.06)" : "#f5f6f8", color: prov.enabled ? "#16a34a" : "#9ca3af" }}>
            ...
            {/* Body */}
            {open && (
                <div style={{ borderTop: "1px solid #ebeef2", padding: "14px", animation: "ttsFade .12s ease-out" }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: "flex", gap: 4 }}>
                                ...
                                <button onClick={() => setShowSecret(!showSecret)} style={{
                                    width: 34, height: 34, borderRadius: 8, border: "1.5px solid #e5e7eb", background: "#fff",
                                    cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#6b7280", flexShrink: 0,
...
export function TTSTab({ agentId }: { agentId: string }) {
    ...
    if (isLoading) return (
        <div style={{ animation: "ttsFade .3s ease-out" }}>
            <style>{CSS}</style>
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, border: "1px solid #ebeef2", background: "#fff", marginBottom: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[0, 1].map(i => <div key={i} style={{ borderRadius: 12, border: "1px solid #ebeef2", background: "#fff", padding: 14 }}><div style={{ display: "flex", gap: 8, alignItems: "center" }}><div style={sk("34px", 34, 8)} /><div><div style={sk("80px", 12)} /><div style={{ ...sk("100px", 9), marginTop: 4 }} /></div></div></div>)}
            </div>
        </div>
    )
    ...
    if (isError && !tts) {
        ...
        return (
            <div style={{ borderRadius: 12, padding: "40px 24px", textAlign: "center", border: "1px solid #ebeef2", background: "#fff" }}>
                <style>{CSS}</style>
                ...
                <div style={{ fontSize: 15, fontWeight: 800, color: "#111827", marginBottom: 12 }}>خطأ في التحميل</div>
    ...
    if (!tts || Object.keys(tts).length === 0) return (
        <div style={{ borderRadius: 12, padding: "48px 24px", textAlign: "center", border: "1.5px dashed #d1d5db" }}>
            <style>{CSS}</style>
            <Volume2 size={28} style={{ margin: "0 auto 10px", display: "block", color: "#d1d5db" }} />
            <div style={{ fontSize: 13, fontWeight: 700, color: "#111827", marginBottom: 3 }}>لم يتم إعداد TTS</div>
            <div style={{ fontSize: 11, color: "#9ca3af" }}>ستظهر الإعدادات بعد تهيئة الخادم</div>
        </div>
    )
        ...
        <div style={{ animation: "ttsFade .25s ease-out" }}>
            ...
            {isRefetching && (
                <div style={{ height: 2, borderRadius: 1, marginBottom: 12, overflow: "hidden", background: "#ebeef2", position: "relative" }}>
                    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(90deg,transparent,#004786,transparent)", animation: "ttsSweep 1.3s ease-in-out infinite" }} />
                </div>
            )}
            ...
            <div style={{
                ...
                <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: tts.enabled ? "linear-gradient(90deg, #004786, #0072b5)" : "#e5e7eb" }} />
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{
                        width: 34, height: 34, borderRadius: 9, flexShrink: 0,
                        background: tts.enabled ? "linear-gradient(135deg, #004786, #0072b5)" : "#f0f1f3",
                        display: "flex", alignItems: "center", justifyContent: "center",
                    }}>
                        <Power size={15} style={{ color: tts.enabled ? "#fff" : "#9ca3af" }} />
                    </div>
                    <div>
                        <div style={{ fontSize: 14, fontWeight: 800, color: "#111827" }}>تحويل النص إلى كلام</div>
                        <div style={{ fontSize: 11, color: "#9ca3af", display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ display: "flex", alignItems: "center", gap: 3 }}>
                                <span style={{ width: 5, height: 5, borderRadius: "50%", background: tts.enabled ? "#16a34a" : "#d1d5db" }} />
                                {tts.enabled ? "يعمل" : "متوقف"}
                            </span>
                            <span>·</span>
                            <span>{enabledCount}/{providerNames.length} مزود</span>
                            {tts.default_provider && <><span>·</span><code style={{ fontFamily: "monospace", fontSize: 10, background: "#f5f6f8", padding: "0 4px", borderRadius: 3 }}>{tts.default_provider}</code></>}
                        </div>
                    </div>
                </div>
            ...
            <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
                    <AudioWaveform size={13} style={{ color: "#004786" }} />
                    <span style={{ fontSize: 13, fontWeight: 800, color: "#111827" }}>مزودو الصوت</span>
                    {providerNames.length > 0 && <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 7px", borderRadius: 20, background: "rgba(0,71,134,.06)", color: "#004786" }}>{providerNames.length}</span>}
                </div>

                {providerNames.length === 0 ? (
                    <div style={{ borderRadius: 12, padding: "28px 0", textAlign: "center", border: "1.5px dashed #d1d5db", color: "#9ca3af" }}>
