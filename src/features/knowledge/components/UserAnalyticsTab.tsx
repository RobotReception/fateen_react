...
const DeleteConfirmModal = memo(function DeleteConfirmModal({
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div className="mx-4 w-full max-w-md overflow-hidden bg-white" onClick={(e) => e.stopPropagation()} style={{ animation: "uaModalIn .18s ease-out", borderRadius: 12, border: "1px solid var(--t-border-light, var(--t-border))" }}>
                <div style={{ padding: "28px 24px 20px", textAlign: "center" }}>
                    ...
                    <p style={{ fontSize: 13, color: "var(--t-text-secondary, var(--t-text-muted))", marginTop: 8 }}>{subtitle}</p>
...
const UserFilesModal = memo(function UserFilesModal({
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div className="mx-4 w-full max-w-3xl max-h-[85vh] flex flex-col bg-white" onClick={(e) => e.stopPropagation()} style={{ animation: "uaModalIn .18s ease-out", borderRadius: 12, border: "1px solid var(--t-border-light, var(--t-border))" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", borderBottom: "1px solid var(--t-border-light, #f0f0f0)", flexShrink: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div>
                            ...
                            <p style={{ fontSize: 10, color: "var(--t-text-faint, var(--t-text-faint))", fontFamily: "monospace", marginTop: 1 }}>{username}</p>
                        </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <button onClick={() => setConfirmDeleteAll(true)} style={{ display: "flex", alignItems: "center", gap: 5, borderRadius: 6, padding: "5px 12px", border: "1px solid rgba(220,38,38,0.2)", background: "rgba(220,38,38,0.04)", fontSize: 11, fontWeight: 500, color: "var(--t-danger)", cursor: "pointer" }}><Ban size={11} />حذف جميع بيانات المستخدم</button>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
                    {loading ? (
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 0" }}><div className="ua-loading-spinner" /><p style={{ marginTop: 12, fontSize: 13, color: "var(--t-text-faint, var(--t-text-faint))" }} className="animate-pulse">جاري تحميل الملفات...</p></div>
                    ) : files.length === 0 ? (
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 0", textAlign: "center" }}><div style={{ width: 52, height: 52, borderRadius: 14, background: "var(--t-surface, var(--t-surface))", display: "flex", alignItems: "center", justifyContent: "center" }}><FolderOpen size={22} style={{ color: "var(--t-text-faint, var(--t-border-medium))" }} /></div><p style={{ marginTop: 12, fontSize: 13, fontWeight: 500, color: "var(--t-text-secondary, var(--t-text-muted))" }}>لا توجد ملفات</p></div>
                    ) : (
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            <p style={{ fontSize: 11, fontWeight: 600, color: "var(--t-text-faint, var(--t-text-faint))", marginBottom: 4 }}>{files.length} ملف</p>
                            {files.map((file, idx) => (
                                <div key={`${file.media_id}-${idx}`}
                                    style={{ borderRadius: 8, border: deletingFile === file.filename ? "1px solid rgba(220,38,38,0.15)" : "1px solid var(--t-border-light, var(--t-border))", background: deletingFile === file.filename ? "rgba(220,38,38,0.02)" : "var(--t-card, #fff)", padding: 14, transition: "all 0.15s", opacity: deletingFile === file.filename ? 0.5 : 1, animation: `uaRowFade 0.3s ease-out ${idx * 0.05}s both` }}>
                                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                                        <div style={{ display: "flex", alignItems: "flex-start", gap: 10, minWidth: 0 }}>
                                            <div style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(27,80,145,0.06)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><FileText size={16} style={{ color: "var(--t-accent)" }} /></div>
                                            <div style={{ minWidth: 0, flex: 1 }}>
                                                <p style={{ fontSize: 13, fontWeight: 600, color: "var(--t-text, var(--t-text-secondary))" }} className="truncate" dir="ltr">{file.filepath || file.filename}</p>
                                                <p style={{ marginTop: 2, fontSize: 10, color: "var(--t-text-faint, var(--t-text-faint))", fontFamily: "monospace" }} className="truncate" dir="ltr">{file.filename}</p>
                                            </div>
                                        </div>
                                        <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                                            <span style={{ display: "flex", alignItems: "center", gap: 4, borderRadius: 12, background: "rgba(27,80,145,0.05)", padding: "3px 8px", fontSize: 10, fontWeight: 600, color: "var(--t-accent)" }}><Database size={9} />{file.id_count} سجل</span>
                                            <button onClick={() => setViewDataFile(file)} style={{ borderRadius: 6, padding: 5, border: "none", background: "transparent", cursor: "pointer", color: "var(--t-text-faint, var(--t-text-faint))", transition: "color 0.12s" }} title="عرض البيانات" onMouseEnter={e => { e.currentTarget.style.color = "var(--t-accent)" }} onMouseLeave={e => { e.currentTarget.style.color = "var(--t-text-faint, var(--t-text-faint))" }}><Eye size={13} /></button>
                                            <button onClick={() => handleDownload(file)} disabled={!!downloadingFile} style={{ borderRadius: 6, padding: 5, border: "none", background: "transparent", cursor: "pointer", color: "var(--t-text-faint, var(--t-text-faint))", transition: "color 0.12s", opacity: downloadingFile ? 0.3 : 1 }} title="تنزيل الملف" onMouseEnter={e => { e.currentTarget.style.color = "#059669" }} onMouseLeave={e => { e.currentTarget.style.color = "var(--t-text-faint, var(--t-text-faint))" }}>
                                                {downloadingFile === file.filename ? <Loader2 size={13} className="animate-spin" style={{ color: "#059669" }} /> : <Download size={13} />}
                                            </button>
                                            <button onClick={() => setConfirmDeleteFile(file)} disabled={!!deletingFile} style={{ borderRadius: 6, padding: 5, border: "none", background: "transparent", cursor: "pointer", color: "var(--t-text-faint, var(--t-border-medium))", transition: "color 0.12s", opacity: deletingFile ? 0.3 : 1 }} title="حذف هذا الملف" onMouseEnter={e => { e.currentTarget.style.color = "var(--t-danger)" }} onMouseLeave={e => { e.currentTarget.style.color = "var(--t-text-faint, var(--t-border-medium))" }}>
                                                {deletingFile === file.filename ? <Loader2 size={13} className="animate-spin" style={{ color: "var(--t-danger)" }} /> : <Trash2 size={13} />}
                                            </button>
                                        </div>
                                    </div>
                                    <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", alignItems: "center", gap: 6 }}>
                                        {file.department_id && <span style={{ display: "flex", alignItems: "center", gap: 4, borderRadius: 12, background: "rgba(27,80,145,0.04)", border: "1px solid rgba(27,80,145,0.08)", padding: "2px 8px", fontSize: 10, color: "var(--t-accent)" }}><Building2 size={8} />{file.department_id}</span>}
                                        {file.category_id && <span style={{ display: "flex", alignItems: "center", gap: 4, borderRadius: 12, background: "rgba(27,80,145,0.04)", border: "1px solid rgba(27,80,145,0.08)", padding: "2px 8px", fontSize: 10, color: "var(--t-accent)" }}><Tag size={8} />{file.category_id}</span>}
                                        {file.media_id && <span style={{ display: "flex", alignItems: "center", gap: 4, borderRadius: 12, background: "var(--t-surface, var(--t-surface))", border: "1px solid var(--t-border-light, var(--t-border))", padding: "2px 8px", fontSize: 10, color: "var(--t-text-faint, var(--t-text-faint))", fontFamily: "monospace" }} dir="ltr"><Hash size={8} />{file.media_id.slice(0, 12)}…</span>}
                                    </div>
                                </div>
                            ))}
                        </div>
...
const UserRow = memo(function UserRow({
    ...
    const colors = ["var(--t-info)", "#8b5cf6", "var(--t-success)", "var(--t-warning)", "var(--t-danger)", "#ec4899", "#06b6d4", "#6366f1", "#14b8a6", "#f97316"]
    ...
    return (
        <tr className="group transition-colors" style={{ animation: `uaRowFade 0.3s ease-out ${idx * 0.04}s both`, background: deletingUser ? "rgba(220,38,38,0.02)" : "transparent", opacity: deletingUser ? 0.5 : 1 }}
            onMouseEnter={e => { if (!deletingUser) e.currentTarget.style.background = "var(--t-card-hover, var(--t-page))" }}
            ...
            <td style={{ padding: "12px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <p style={{ fontSize: 13, fontWeight: 500, color: "var(--t-text, var(--t-text-secondary))" }} className="truncate" dir="ltr">{user.username}</p>
                </div>
            </td>
            <td style={{ padding: "12px 16px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 5, borderRadius: 12, background: "rgba(27,80,145,0.05)", padding: "3px 10px", fontSize: 11, fontWeight: 600, color: "var(--t-accent)" }}><FileText size={11} />{user.file_count}</span></td>
            <td style={{ padding: "12px 16px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 5, borderRadius: 12, background: "rgba(27,80,145,0.05)", padding: "3px 10px", fontSize: 11, fontWeight: 600, color: "var(--t-accent)" }}><Database size={11} />{user.total_ids.toLocaleString()}</span></td>
            <td style={{ padding: "12px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <ActionGuard pageBit={PAGE_BITS.DOCUMENTS} actionBit={ACTION_BITS.LIST_USER_FILES}>
                        <button onClick={() => onViewFiles(user.username)} style={{ display: "flex", alignItems: "center", gap: 5, borderRadius: 6, padding: "5px 12px", border: "1px solid var(--t-border-light, var(--t-border))", background: "var(--t-card, #fff)", fontSize: 11, fontWeight: 500, color: "var(--t-text-secondary, var(--t-text-muted))", cursor: "pointer", transition: "all 0.12s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "var(--t-accent)"; e.currentTarget.style.color = "var(--t-accent)" }} onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--t-border-light, var(--t-border))"; e.currentTarget.style.color = "var(--t-text-secondary, var(--t-text-muted))" }}><Eye size={12} />عرض الملفات</button>
                    </ActionGuard>
                    <ActionGuard pageBit={PAGE_BITS.DOCUMENTS} actionBit={ACTION_BITS.DELETE_DOCS_BY_USER}>
                        <button onClick={() => onDeleteUser(user)} style={{ borderRadius: 6, padding: 5, border: "none", background: "transparent", cursor: "pointer", color: "var(--t-text-faint, var(--t-border-medium))", transition: "color 0.12s", opacity: 0 }} className="group-hover:!opacity-100" title="حذف جميع بيانات المستخدم" onMouseEnter={e => { e.currentTarget.style.color = "var(--t-danger)" }} onMouseLeave={e => { e.currentTarget.style.color = "var(--t-text-faint, var(--t-border-medium))" }}><Trash2 size={13} /></button>
                    </ActionGuard>
                </div>
            </td>
        </tr>
...
export function UserAnalyticsTab() {
    ...
    return (
        <div className="space-y-4">
            ...
            <div style={{ borderRadius: 10, border: "1px solid var(--t-border-light, var(--t-border))", background: "var(--t-card, #fff)", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderBottom: "1px solid var(--t-border-light, #f0f0f0)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 36, height: 36, borderRadius: 9, background: "var(--t-gradient-accent)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <Users size={17} style={{ color: "#fff" }} />
                        </div>
                        <div>
                            <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--t-text, #1f2937)", margin: 0 }}>ملفات المستخدمين</h2>
                            <p style={{ fontSize: 11, color: "var(--t-text-faint, var(--t-text-faint))", marginTop: 1 }}>عرض المستخدمين وملفاتهم في قاعدة المعرفة</p>
                        </div>
                    </div>
                    {!loading && users.length > 0 && (
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, borderRadius: 12, background: "var(--t-brand-orange-muted)", border: "1px solid var(--t-brand-orange-soft)", padding: "4px 12px", fontSize: 11, fontWeight: 600, color: "var(--t-brand-orange)" }}>
                            <Users size={11} />
                            {totalCount} مستخدم
                        </span>
                    )}
                ...
                <div className="overflow-x-auto" style={{ padding: "10px 18px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <button onClick={() => handleDeptChange("")}
                            style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 5, borderRadius: 7, padding: "6px 12px", fontSize: 11, fontWeight: !selectedDept ? 600 : 500, border: !selectedDept ? "1px solid var(--t-brand-orange)" : "1px solid var(--t-border-light, var(--t-border))", background: !selectedDept ? "var(--t-brand-orange-muted)" : "transparent", color: !selectedDept ? "var(--t-brand-orange)" : "var(--t-text-secondary, var(--t-text-muted))", cursor: "pointer", transition: "all 0.15s" }}>
                            <Building2 size={11} />الكل
                        </button>
                        {departments.map((dept) => (
                            <button key={dept.department_id} onClick={() => handleDeptChange(dept.department_id)}
                                style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 5, borderRadius: 7, padding: "6px 12px", fontSize: 11, fontWeight: selectedDept === dept.department_id ? 600 : 500, border: selectedDept === dept.department_id ? "1px solid var(--t-brand-orange)" : "1px solid var(--t-border-light, var(--t-border))", background: selectedDept === dept.department_id ? "var(--t-brand-orange-muted)" : "transparent", color: selectedDept === dept.department_id ? "var(--t-brand-orange)" : "var(--t-text-secondary, var(--t-text-muted))", cursor: "pointer", transition: "all 0.15s" }}>
            ...
            <div className="flex items-center gap-3">
                <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="بحث بالبريد الإلكتروني..." value={searchInput} onChange={(e) => handleSearchInput(e.target.value)} style={{ width: "100%", borderRadius: 8, border: "1px solid var(--t-border-light, var(--t-border))", background: "var(--t-card, #fff)", padding: "8px 36px 8px 36px", fontSize: 13, color: "var(--t-text, var(--t-text-secondary))", outline: "none" }} dir="ltr" />
                ...
                <div className="relative shrink-0">
                    ...
                    <select value={selectedCat} onChange={(e) => handleCatChange(e.target.value)}
                        style={{ appearance: "none", borderRadius: 8, border: selectedCat ? "1px solid rgba(27,80,145,0.3)" : "1px solid var(--t-border-light, var(--t-border))", background: selectedCat ? "rgba(27,80,145,0.04)" : "var(--t-card, #fff)", padding: "8px 16px 8px 32px", fontSize: 12, fontWeight: 500, color: selectedCat ? "var(--t-accent)" : "var(--t-text-secondary, var(--t-text-muted))", outline: "none", cursor: "pointer", minWidth: 130 }}>
                ...
                <ActionGuard pageBit={PAGE_BITS.DOCUMENTS} actionBit={ACTION_BITS.DELETE_COLLECTION}>
                    <button onClick={() => setShowDeleteCollection(true)} style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 5, padding: "8px 14px", borderRadius: 8, border: "1px solid rgba(220,38,38,0.15)", background: "rgba(220,38,38,0.03)", color: "var(--t-danger)", fontSize: 12, fontWeight: 500, cursor: "pointer", transition: "all 0.15s" }}><Ban size={13} />حذف الكل</button>
                </ActionGuard>
            </div>

            {/* Info bar */}
            {!loading && users.length > 0 && (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderRadius: 8, background: "var(--t-card, #fff)", border: "1px solid var(--t-border-light, var(--t-border))", padding: "7px 14px" }}>
                    <p style={{ fontSize: 11, color: "var(--t-text-secondary, var(--t-text-muted))" }}>
                        <span style={{ fontWeight: 700, color: "var(--t-brand-orange)" }}>{totalCount}</span> مستخدم
                        {selectedDept && <span style={{ color: "var(--t-text-faint, var(--t-text-faint))" }}> — {activeDeptLabel}</span>}
                    </p>
            ...
            <div style={{ position: "relative", overflow: "hidden", borderRadius: 10, border: "1px solid var(--t-border-light, var(--t-border))", background: "var(--t-card, #fff)" }}>
                ...
                {!loading && users.length === 0 && (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "64px 0", textAlign: "center" }}>
                        <div style={{ width: 56, height: 56, borderRadius: 14, background: "var(--t-surface, var(--t-surface))", display: "flex", alignItems: "center", justifyContent: "center" }}><User size={24} style={{ color: "var(--t-text-faint, var(--t-border-medium))" }} /></div>
                        <p style={{ marginTop: 14, fontSize: 13, fontWeight: 500, color: "var(--t-text-secondary, var(--t-text-muted))" }}>{searchVal ? "لا يوجد مستخدمين مطابقين" : selectedDept ? `لا يوجد ملفات في قسم ${activeDeptLabel}` : "لا يوجد مستخدمين"}</p>
                        <p style={{ marginTop: 4, fontSize: 11, color: "var(--t-text-faint, var(--t-text-faint))" }}>{searchVal ? "جرب كلمة بحث مختلفة" : "لم يتم رفع أي ملفات بعد"}</p>
                    </div>
                )}
            </div>
            ...
            {totalPages > 1 && (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, paddingTop: 8 }}>
                    ...
                    <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} onMouseEnter={() => page < totalPages && prefetchNextUsers()} disabled={page >= totalPages} style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: 7, border: "1px solid var(--t-border-light, var(--t-border))", background: "var(--t-card, #fff)", fontSize: 12, fontWeight: 500, color: "var(--t-text-secondary, var(--t-text-muted))", cursor: "pointer", opacity: page >= totalPages ? 0.4 : 1 }}>التالي <ChevronLeft size={13} /></button>
                </div>
            )}
                    ...
                    <div className="mx-4 w-full max-w-md overflow-hidden bg-white" onClick={(e) => e.stopPropagation()} style={{ animation: "uaModalIn .18s ease-out", borderRadius: 12, border: "1px solid var(--t-border-light, var(--t-border))" }}>
                        <div style={{ padding: 20 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                                <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(220,38,38,0.06)", display: "flex", alignItems: "center", justifyContent: "center" }}><Ban size={18} style={{ color: "var(--t-danger)" }} /></div>
                                <div><h3 style={{ fontSize: 14, fontWeight: 600, color: "var(--t-text, #1f2937)" }}>حذف جميع البيانات</h3><p style={{ fontSize: 11, color: "var(--t-text-faint, var(--t-text-faint))", marginTop: 1 }}>سيتم حذف جميع البيانات من قاعدة المعرفة</p></div>
                            </div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, borderTop: "1px solid var(--t-border-light, #f0f0f0)", padding: "12px 20px" }}>
                            <button onClick={() => setShowDeleteCollection(false)} style={{ padding: "7px 16px", borderRadius: 7, border: "1px solid var(--t-border-light, var(--t-border))", background: "var(--t-card, #fff)", fontSize: 13, fontWeight: 500, color: "var(--t-text-secondary, var(--t-text-muted))", cursor: "pointer" }}>إلغاء</button>
                            <button onClick={handleDeleteCollection} disabled={deletingCollection} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 18px", borderRadius: 7, border: "none", background: "var(--t-danger)", color: "#fff", fontSize: 13, fontWeight: 500, cursor: "pointer", opacity: deletingCollection ? 0.5 : 1 }}>
            ...
            <style>{`
                ...
                .ua-skeleton-bone{background:linear-gradient(90deg,var(--t-surface) 25%,var(--t-border) 37%,var(--t-surface) 63%);background-size:200% 100%;animation:uaShimmer 1.5s ease-in-out infinite}
                .ua-skeleton-shimmer{opacity:0;animation:uaFadeIn .3s ease-out forwards}
                .ua-loading-spinner{width:36px;height:36px;border:3px solid var(--t-border);border-top-color:var(--t-info);border-radius:50%;animation:uaSpin .7s linear infinite}
